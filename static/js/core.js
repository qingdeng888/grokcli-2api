...
(function () {
  ...
  let regRestoreInFlight = false;
  let regLastRestoreAt = 0;
  let regLastLogText = "";
...
async function softNavigate(name, opts) {
  ...
  try {
    ...
    if (page === "accounts") {
      ...
      try { await restoreRegProgress(); } catch (e) { console.warn("restoreRegProgress", e); }
    }
...
function rebindPageControls() {
  ...
  if ($("reg-mail-provider")) {
    on("reg-mail-provider", "onchange", () => syncRegMailProviderUI());
    syncRegMailProviderUI();
  }
  bindRegMailPoolControls();
  if ($("reg-captcha-provider")) {
    on("reg-captcha-provider", "onchange", () => syncRegCaptchaProviderUI());
    syncRegCaptchaProviderUI();
  }
  if ($("btn-refresh-reg")) on("btn-refresh-reg", "onclick", async () => {
    if (regBatchId || regSessionId || (regSessionIds && regSessionIds.length)) {
      showPanel("reg-session-box");
      await pollRegSession();
    } else {
      const restored = await restoreRegProgress({ force: true });
      if (!restored) toast("没有可恢复的注册日志", false);
    }
  });
  ...
  if ($("btn-refresh-reg-inline")) on("btn-refresh-reg-inline", "onclick", async () => {
    if (regBatchId || regSessionId || (regSessionIds && regSessionIds.length)) await pollRegSession();
    else await restoreRegProgress({ force: true });
  });
...
const REG_PROGRESS_RETENTION_SEC = 6 * 3600;
...
function regProgressTimestamp(item) {
  const ts = Number((item && (item.updated_at || item.created_at)) || 0);
  return Number.isFinite(ts) ? ts : 0;
}

function isRecentRegProgress(item) {
  const ts = regProgressTimestamp(item);
  return ts > 0 && Date.now() / 1000 - ts <= REG_PROGRESS_RETENTION_SEC;
}

function isActiveRegSession(session) {
  const status = regStatusOf(session);
  return !!(
    session && session.register_delay_active
  ) || (!REG_TERMINAL_OK.has(status) && !REG_TERMINAL_BAD.has(status));
}

function isActiveRegBatch(batch) {
  const status = String((batch && (batch.batch_status || batch.status)) || "").toLowerCase();
  if (
    batch && (Number(batch.running) > 0 || Number(batch.delay_waiting) > 0)
  ) return true;
  return !!status && !["done", "partial", "error", "cancelled", "stopped"].includes(status);
}

async function restoreRegProgress({ force = false } = {}) {
  const page = document.body.dataset.page || pageFromPath(location.pathname) || "";
  if (page !== "accounts") return false;
  if (regRestoreInFlight) return false;
  if (!force && Date.now() - regLastRestoreAt < 1500) return false;
  regRestoreInFlight = true;
  regLastRestoreAt = Date.now();
  try {
    const data = await api("/accounts/register-email/sessions");
    const sessions = Array.isArray(data && data.sessions) ? data.sessions.slice() : [];
    const batches = Array.isArray(data && data.batches) ? data.batches.slice() : [];
    sessions.sort((a, b) => regProgressTimestamp(b) - regProgressTimestamp(a));
    batches.sort((a, b) => regProgressTimestamp(b) - regProgressTimestamp(a));

    const batch = batches.find(isActiveRegBatch) || batches.find(isRecentRegProgress) || null;
    if (batch) {
      const batchId = String(batch.id || batch.batch_id || "").trim();
      const ids = Array.isArray(batch.session_ids)
        ? batch.session_ids.map(String).filter(Boolean)
        : [];
      const matched = sessions.filter((session) => {
        const id = regSessionKey(session);
        return (batchId && session.batch_id === batchId) || (id && ids.includes(id));
      });
      for (const session of matched) {
        const id = regSessionKey(session);
        if (id && !ids.includes(id)) ids.push(id);
      }
      regBatchId = batchId || null;
      regSessionIds = ids;
      regSessionId = ids[0] || null;
      regStopping = String(batch.status || batch.batch_status || "").toLowerCase() === "stopping";
      const active = isActiveRegBatch(batch) || matched.some(isActiveRegSession);
      regFinishedNotified = !active;
      regPollInFlight = false;
      regLastLogText = "";
      showPanel("reg-session-box");
      showRegSessionGroup(matched, { batch });
      if (active) startRegPolling({ immediate: true, intervalMs: 2000 });
      else {
        try { clearInterval(regPollTimer); } catch (_) {}
        regPollTimer = null;
      }
    ...
    const latest = sessions.find(isActiveRegSession) || sessions.find(isRecentRegProgress) || null;
    if (!latest) return false;
    const batchId = String(latest.batch_id || "").trim();
    const matched = batchId
      ? sessions.filter((session) => session.batch_id === batchId)
      : [latest];
    regBatchId = batchId || null;
    regSessionIds = matched.map(regSessionKey).filter(Boolean);
    regSessionId = regSessionIds[0] || regSessionKey(latest) || null;
    const active = matched.some(isActiveRegSession);
    regFinishedNotified = !active;
    regStopping = matched.some((session) => regStatusOf(session) === "stopping");
    regPollInFlight = false;
    regLastLogText = "";
    showPanel("reg-session-box");
    if (matched.length > 1 || regBatchId) showRegSessionGroup(matched);
    else showRegSession(latest);
    if (active) startRegPolling({ immediate: true, intervalMs: 2000 });
    return true;
  } catch (error) {
    console.warn("restoreRegProgress", error);
    return false;
  } finally {
    regRestoreInFlight = false;
  }
}
...
let regMailKeys = { moemail: "", yyds: "", gptmail: "", cfmail: "", inbucket: "" };
let regMailDomains = { moemail: "", yyds: "", gptmail: "", cfmail: "", inbucket: "" };
// Self-hosted hosts are isolated so switching providers never overwrites another host.
let regMailBaseUrls = { moemail: "", cfmail: "", inbucket: "" };
let regMailProviderPrev = "moemail";
let regMailProviders = [];
let regMailProviderSelectedId = "";

function splitRegMailDomains(value) {
  const out = [];
  String(value || "").split(/[\n,]/).forEach((item) => {
    const domain = item.trim().replace(/^@+/, "").replace(/^\.+|\.+$/g, "");
    if (domain && !out.includes(domain)) out.push(domain);
  });
  return out;
}

function currentRegMailEntry() {
  return regMailProviders.find((item) => item.id === regMailProviderSelectedId) || null;
}

function stashCurrentRegMailEntry() {
  const entry = currentRegMailEntry();
  if (!entry) return;
  entry.label = $("reg-mail-label") ? ($("reg-mail-label").value || "").trim() : entry.label;
  entry.enabled = $("reg-mail-enabled") ? !!$("reg-mail-enabled").checked : true;
  entry.type = currentRegMailProvider();
  entry.base_url = $("reg-base-url") ? ($("reg-base-url").value || "").trim() : "";
  entry.api_key = $("reg-api-key") ? ($("reg-api-key").value || "").trim() : "";
  entry.domains = splitRegMailDomains($("reg-domain") ? $("reg-domain").value : "");
}

function renderRegMailProviderList() {
  const box = $("reg-mail-provider-list");
  if (!box) return;
  box.innerHTML = "";
  regMailProviders.forEach((entry, index) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = `g2a-btn g2a-btn-sm ${entry.id === regMailProviderSelectedId ? "g2a-btn-primary" : "g2a-btn-default"}`;
    btn.textContent = `${entry.enabled === false ? "停用 · " : ""}${entry.label || `${entry.type}#${index + 1}`} · ${(entry.domains || []).length || "自动域名"}`;
    btn.onclick = () => {
      stashCurrentRegMailEntry();
      regMailProviderSelectedId = entry.id;
      applySelectedRegMailEntry();
    };
    box.appendChild(btn);
  });
  const enabled = regMailProviders.filter((item) => item.enabled !== false);
  const domains = enabled.reduce((sum, item) => sum + ((item.domains || []).length || 0), 0);
  if ($("reg-mail-pool-summary")) $("reg-mail-pool-summary").textContent = `启用 ${enabled.length} · 域名 ${domains || "自动"}`;
}

function applySelectedRegMailEntry() {
  const entry = currentRegMailEntry();
  if (!entry) {
    if ($("reg-mail-label")) $("reg-mail-label").value = "";
    if ($("reg-mail-enabled")) $("reg-mail-enabled").checked = false;
    if ($("reg-api-key")) $("reg-api-key").value = "";
    if ($("reg-domain")) $("reg-domain").value = "";
    if ($("reg-base-url")) $("reg-base-url").value = "";
    renderRegMailProviderList();
    return;
  }
  if ($("reg-mail-provider")) $("reg-mail-provider").value = entry.type || "moemail";
  regMailProviderPrev = currentRegMailProvider();
  regMailKeys[regMailProviderPrev] = entry.api_key || "";
  regMailDomains[regMailProviderPrev] = (entry.domains || []).join("\n");
  if (regMailProviderPrev === "moemail" || regMailProviderPrev === "cfmail" || regMailProviderPrev === "inbucket") {
    regMailBaseUrls[regMailProviderPrev] = entry.base_url || "";
  }
  syncRegMailProviderUI();
  if ($("reg-mail-label")) $("reg-mail-label").value = entry.label || "";
  if ($("reg-mail-enabled")) $("reg-mail-enabled").checked = entry.enabled !== false;
  if ($("reg-api-key")) $("reg-api-key").value = entry.api_key || "";
  if ($("reg-domain")) $("reg-domain").value = (entry.domains || []).join("\n");
  if ($("reg-base-url")) $("reg-base-url").value = entry.base_url || "";
  renderRegMailProviderList();
}

function ensureRegMailProviders(cfg) {
  const hasPool = !!(cfg && Array.isArray(cfg.mail_providers));
  const source = hasPool ? cfg.mail_providers : [];
  regMailProviders = source.map((item, index) => ({
    id: String(item.id || `mail_${Date.now()}_${index}`),
    label: String(item.label || `${item.type || "moemail"}#${index + 1}`),
    type: String(item.type || "moemail"),
    enabled: item.enabled !== false,
    base_url: String(item.base_url || ""),
    api_key: String(item.api_key || ""),
    domains: Array.isArray(item.domains) ? item.domains.map(String).filter(Boolean) : splitRegMailDomains(item.domains),
  }));
  if (!hasPool) {
    regMailProviders = [{
      id: "mail_legacy_primary",
      label: `${String((cfg && cfg.mail_provider) || "moemail").toUpperCase()}-1`,
      type: String((cfg && cfg.mail_provider) || "moemail"),
      enabled: true,
      base_url: String((cfg && cfg.base_url) || ""),
      api_key: String((cfg && cfg.api_key) || ""),
      domains: splitRegMailDomains((cfg && cfg.domain) || ""),
    }];
  }
  regMailProviderSelectedId = regMailProviders.some((item) => item.id === regMailProviderSelectedId)
    ? regMailProviderSelectedId
    : (regMailProviders[0] ? regMailProviders[0].id : "");
}

function bindRegMailPoolControls() {
  if ($("btn-add-reg-mail")) {
    on("btn-add-reg-mail", "onclick", () => {
      stashCurrentRegMailEntry();
      const id = `mail_${Date.now()}_${Math.random().toString(16).slice(2, 8)}`;
      regMailProviders.push({
        id,
        label: `Inbucket-${regMailProviders.length + 1}`,
        type: "inbucket",
        enabled: true,
        base_url: "",
        api_key: "",
      ...
      regMailProviderSelectedId = id;
      applySelectedRegMailEntry();
      if ($("reg-mail-label")) $("reg-mail-label").focus();
    });
  }
  if ($("btn-delete-reg-mail")) {
    on("btn-delete-reg-mail", "onclick", () => {
      const entry = currentRegMailEntry();
      if (!entry || !confirm(`删除邮箱接口“${entry.label || entry.type}”？`)) return;
      regMailProviders = regMailProviders.filter((item) => item.id !== entry.id);
      regMailProviderSelectedId = regMailProviders[0] ? regMailProviders[0].id : "";
      applySelectedRegMailEntry();
    });
  }
  if ($("reg-mail-provider")) {
    on("reg-mail-provider", "onchange", () => {
      const entry = currentRegMailEntry();
      if (entry) entry.type = currentRegMailProvider();
      syncRegMailProviderUI();
      stashCurrentRegMailEntry();
      renderRegMailProviderList();
    });
  }
  ["reg-mail-label", "reg-mail-enabled", "reg-base-url", "reg-api-key", "reg-domain"].forEach((id) => {
    if (!$(id)) return;
    const eventName = id === "reg-mail-enabled" ? "onchange" : "oninput";
    on(id, eventName, () => {
      stashCurrentRegMailEntry();
      renderRegMailProviderList();
    });
...
function currentRegMailProvider() {
  ...
  if (mail === "inbucket") return "inbucket";
  return "moemail";
}
...
function stashRegMailFieldsFromInput() {
  ...
  if ($("reg-base-url") && (mail === "moemail" || mail === "cfmail" || mail === "inbucket")) {
...
function syncRegMailProviderUI() {
  ...
  const isInbucket = mail === "inbucket";
  ...
  if ($("reg-base-url-wrap")) {
    $("reg-base-url-wrap").style.display = (isMoe || isCf || isInbucket) ? "" : "none";
  }
...
function readRegConfig() {
  ...
  const activeDomain = splitRegMailDomains(regMailDomains[mailProvider] || "")[0] || "";
  stashCurrentRegMailEntry();
  // Keep self-hosted hosts in dedicated slots so save never mixes them.
  if (mailProvider === "moemail" || mailProvider === "cfmail" || mailProvider === "inbucket") {
    regMailBaseUrls[mailProvider] = $("reg-base-url") ? ($("reg-base-url").value || "").trim() : (regMailBaseUrls[mailProvider] || "");
  }
  const activeBase =
    mailProvider === "moemail" || mailProvider === "cfmail" || mailProvider === "inbucket"
      ? (regMailBaseUrls[mailProvider] || "")
      : "";
  return {
    mail_providers: regMailProviders.map((item) => ({
      id: item.id,
      label: item.label,
      type: item.type,
      enabled: item.enabled !== false,
    ...
    inbucket_base_url: regMailBaseUrls.inbucket || "",
    domain: activeDomain,
    moemail_domain: splitRegMailDomains(regMailDomains.moemail || "")[0] || "",
    yyds_domain: splitRegMailDomains(regMailDomains.yyds || "")[0] || "",
    gptmail_domain: splitRegMailDomains(regMailDomains.gptmail || "")[0] || "",
    cfmail_domain: splitRegMailDomains(regMailDomains.cfmail || "")[0] || "",
    inbucket_domain: splitRegMailDomains(regMailDomains.inbucket || "")[0] || "",
    ...
    inbucket_api_key: regMailKeys.inbucket || "",
...
function registrationProxyUrl(cfg) {
  let proxy = String((cfg && cfg.proxy) || "").trim();
  const username = String((cfg && cfg.proxy_username) || "").trim();
  const password = String((cfg && cfg.proxy_password) || "").trim();
  if (!proxy || !username || proxy.includes("@")) return proxy;
  if (!proxy.includes("://")) proxy = `http://${proxy}`;
  const splitAt = proxy.indexOf("://");
  const scheme = proxy.slice(0, splitAt + 3);
  const target = proxy.slice(splitAt + 3);
  const auth = password
    ? `${encodeURIComponent(username)}:${encodeURIComponent(password)}`
    : encodeURIComponent(username);
  return `${scheme}${auth}@${target}`;
}

function applyRegConfig(cfg) {
  if (!cfg || typeof cfg !== "object") return;
  const mail = String(cfg.mail_provider || cfg.provider || "moemail").trim().toLowerCase();
  const mailProv = mail === "yyds" ? "yyds" : mail === "gptmail" ? "gptmail" : mail === "cfmail" ? "cfmail" : mail === "inbucket" ? "inbucket" : "moemail";
  if ($("reg-mail-provider")) {
  ...
  regMailKeys = {
    ...
    inbucket:
      cfg.inbucket_api_key != null && cfg.inbucket_api_key !== ""
        ? String(cfg.inbucket_api_key)
        : (mailProv === "inbucket" ? activeKey : (regMailKeys.inbucket || "")),
  };
  ...
  regMailDomains = {
    ...
    inbucket: pickDomain("inbucket_domain", mailProv === "inbucket"),
  };
  ...
  if (mailProv === "inbucket" && Object.prototype.hasOwnProperty.call(cfg, "inbucket_domain")) {
    regMailDomains.inbucket = cfg.inbucket_domain == null ? "" : String(cfg.inbucket_domain);
  }
  ...
  regMailBaseUrls = {
    ...
    inbucket: pickBase("inbucket_base_url", mailProv === "inbucket"),
  };
  ...
  if (mailProv === "inbucket" && Object.prototype.hasOwnProperty.call(cfg, "inbucket_base_url")) {
    regMailBaseUrls.inbucket = cfg.inbucket_base_url == null ? "" : String(cfg.inbucket_base_url);
  }
  if ($("reg-base-url")) {
    $("reg-base-url").value = (mailProv === "moemail" || mailProv === "cfmail" || mailProv === "inbucket")
  ...
  if ($("reg-proxy")) $("reg-proxy").value = registrationProxyUrl(cfg);
...
async function saveRegConfig() {
  ...
  try {
    ...
    } else if (mail === "inbucket") {
      if (!Object.prototype.hasOwnProperty.call(saved, "inbucket_domain")) saved.inbucket_domain = cfg.inbucket_domain || "";
      if (!Object.prototype.hasOwnProperty.call(saved, "domain")) saved.domain = cfg.domain || "";
      if (!Object.prototype.hasOwnProperty.call(saved, "inbucket_api_key")) saved.inbucket_api_key = cfg.inbucket_api_key || "";
      if (!Object.prototype.hasOwnProperty.call(saved, "api_key")) saved.api_key = cfg.api_key || "";
      if (!Object.prototype.hasOwnProperty.call(saved, "base_url")) saved.base_url = cfg.base_url || "";
      if (!Object.prototype.hasOwnProperty.call(saved, "inbucket_base_url")) saved.inbucket_base_url = cfg.inbucket_base_url || cfg.base_url || "";
      if (!Object.prototype.hasOwnProperty.call(saved, "moemail_base_url")) saved.moemail_base_url = cfg.moemail_base_url || "";
...
function buildRegBody(config) {
  const body = {};
  body.mail_providers = Array.isArray(config.mail_providers) ? config.mail_providers : [];
  ...
  // Self-hosted providers need base_url; YYDS/GPTMail use fixed hosts.
  ...
  body.inbucket_base_url = config.inbucket_base_url == null ? "" : String(config.inbucket_base_url);
  ...
  } else if (body.mail_provider === "inbucket") {
    body.base_url = body.inbucket_base_url || (config.base_url == null ? "" : String(config.base_url));
    body.inbucket_base_url = body.base_url;
  }
  // Always send domain for the active provider (empty clears/auto).
  body.domain = splitRegMailDomains(config.domain == null ? "" : String(config.domain))[0] || "";
  ...
  if (body.mail_provider === "moemail") {
    ...
    body.inbucket_api_key = config.inbucket_api_key == null ? body.api_key : String(config.inbucket_api_key);
    body.inbucket_domain = config.inbucket_domain == null ? body.domain : String(config.inbucket_domain);
  }
...
function formatRegDelayLogLine(s, idx) {
  const delayLabel = formatRegDelayState(s);
  if (!delayLabel) return "";
  const status = regStatusOf(s);
  const result = REG_TERMINAL_OK.has(status)
    ? "注册完成"
    : REG_TERMINAL_BAD.has(status)
      ? "注册失败"
      : "注册结束";
  const email = String((s && s.email) || "—");
  if (s && s.register_delay_active) {
    return `[倒计时] #${idx + 1} ${email} ${result}，${delayLabel}；倒计时结束后继续注册`;
  }
  if (s && s.register_delay_completed === false) {
    return `[延时中断] #${idx + 1} ${email} ${result}，${delayLabel}`;
  }
  return `[延时完成] #${idx + 1} ${email} ${result}，${delayLabel}`;
}
...
function formatRegSessionLine(s, idx) {
  ...
  const proxyDisplay = String((s && s.proxy_display) || "").trim();
  const outboundIp = String((s && s.outbound_ip) || "").trim();
  const outboundIpError = String((s && s.outbound_ip_error) || "").trim();
  let networkTxt = "";
  ...
  if (proxyDisplay || outboundIp || outboundIpError) {
    const route = !proxyDisplay || proxyDisplay === "直连"
      ? "直连"
      : `代理 ${proxyDisplay}`;
    const exit = outboundIp && outboundIp !== "unknown"
      ? outboundIp
      : `未获取${outboundIpError ? `（${outboundIpError.slice(0, 80)}）` : ""}`;
    networkTxt = ` | 网络 ${route} → 出口 ${exit}`;
  }
...
function buildRegLogText(sessions, { batch = null, extraLines = [] } = {}) {
  ...
  const delayRows = (sessions || [])
    .map((session, index) => formatRegDelayLogLine(session, index))
    .filter(Boolean);
  if (delayRows.length) {
    lines.push("-------- 注册间隔倒计时 --------");
    lines.push(...delayRows);
  }
  lines.push("-------- 会话明细 --------");
  ...
  try {
    ...
    const batchDone =
      ...
      (batchStatus === "done" ||
        ...
        batchStatus === "stopped" ||
        (Number(batch.done) > 0 &&
          Number(batch.done) >= Number(batch.total || batch.count || 0) &&
          Number(batch.running || 0) <= 0 &&
          Number(batch.delay_waiting || 0) <= 0));
    const batchStopping =
      !!regStopping ||
    ...
    const allTerminal =
      ...
      sessions.every(
        (s) =>
          !s.register_delay_active &&
          (REG_TERMINAL_OK.has(regStatusOf(s)) || REG_TERMINAL_BAD.has(regStatusOf(s)))
      );
    // Prefer batch-level completion: large batches may only keep a compact session window in UI.
    const finished =
      !!batchDone ||
      (allTerminal &&
        (!regBatchId || !batch || batchStopping));
...
if ($("btn-refresh-reg") && !$("btn-refresh-reg").onclick) {
  on("btn-refresh-reg", "onclick", async () => {
    if (regBatchId || regSessionId || (regSessionIds && regSessionIds.length)) {
      showPanel("reg-session-box");
      await pollRegSession();
    } else {
...
if ($("btn-refresh-reg-inline") && !$("btn-refresh-reg-inline").onclick) {
  on("btn-refresh-reg-inline", "onclick", async () => {
    if (regBatchId || regSessionId || (regSessionIds && regSessionIds.length)) await pollRegSession();
    else await restoreRegProgress({ force: true });
  });
}
